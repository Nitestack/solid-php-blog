...
const UpdateBlogOverviewPage: Component = () => {
    ...
    return (
        <Show when={!response.loading}>
            ...
            <Show when={response.blogs}>
                ...
                <Description>{translate("UPDATE_BLOG_OVERVIEW_DESCRIPTION")}</Description>
                <div class="flex items-center justify-center">
                    <div class="grid gap-8 lg:grid-cols-2">
                        <For each={Object.keys(response.blogs)}>
                            {key => (
                                <BlogPreview onClick={() => navigate(`/admin/update/${key}`)} key={key} blog={response.blogs[key]} />
                            )}
                        </For>
                    </div>
                </div>
...
export default UpdateBlogOverviewPage;